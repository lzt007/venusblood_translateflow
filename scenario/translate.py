...
def run(keyjson_dir,translated_dir,roles_path,subTitles_path,sub_role):
    ...
    for root, dirs, files in os.walk(keyjson_dir):
        for file in files:
            try:
                ...
                print("正在翻译选择支")
                for i in range(0, len(data_list)):
                    if len(data_list["select"]) > 0:
                        for j in range(0, len(data_list["select"])):
                            data_list["select"][j]["text"] = translate(
                                data_list["select"][j]["text"],
                                {},
                                role=sub_role
                            )
                ...
                for i in range(0, len(data_list)):
                    ...
                    for j in range(0, len(data_list[i]["window"]["texts"])):
                        data_list[i]["window"]["texts"][j] = translate(
                            data_list[i]["window"]["texts"][j],
                            {},
                            role=data_list[i]["window"]["name"]
                        )
                    print(data_list[i])
